...
import com.lee.activity.dao.ActRepository;
...
public class ActService {
    ...
    public ActVO updateAct(Integer id, ActVO actVO) {
        ActVO act = actRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Activity not found for this id : " + id));
        // 更新其他屬性
    ...
    public void deleteAct(Integer id) {
        // 方法接受一個ID，用於刪除特定的活動
        ActVO act = actRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Activity not found for this id : " + id));
